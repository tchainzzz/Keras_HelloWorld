...
class NNClassifier(AbstractClassifier):
    def __init__(self, dataset=keras.datasets.fashion_mnist):
        ...
        self.n_classes = max(len(np.unique(self.test_labels)), len(np.unique(self.train_labels)))
        self.model = None
    ...
    def build_model(self, layer1_size=128, layer2_size=10, layer1_act=tf.nn.relu, layer2_act=tf.nn.softmax):
    ...
    def eval(self):
        ...
        self.plot_predictions(['T-shirt/top', 'Trouser', 'Pullover', 'Dress', 'Coat',
                 'Sandal', 'Shirt', 'Sneaker', 'Bag', 'Ankle boot'])

    def plot_predictions(self, classlabels=None):
         class_names = classlabels
         if class_names is None:
            class_names = range(self.n_classes)
